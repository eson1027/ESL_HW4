	...
	for (int j = 0; j < width; j++) {
		for (int i = -2; i < height + 2; i++) {
			...
			for (int v = -2; v <= 2; v++) {
				// for(int u = -2; u <= 2; u++){
				if ((v + j) >= 0 && (v + j) < height) {
					buffer[0] = *(source_bitmap + bytes_per_pixel * ((j + v) * width + (i)) + 2);
					buffer[1] = *(source_bitmap + bytes_per_pixel * ((j + v) * width + (i)) + 1);
					buffer[2] = *(source_bitmap + bytes_per_pixel * ((j + v) * width + (i)) + 0);
					buffer[3] = 0;
				} else {
					buffer[0] = 0;
					buffer[1] = 0;
			...
			// unsigned char flt_to_tb[4] = {0};
			read_data_from_ACC(SOBELFILTER_READ_ADDR, buffer, 4);
			// if (flt_to_tb[3] != 0) {
			// 	std::cout << int(flt_to_tb[3]) << std::endl;
			// }
			...
			// total = (data).sint;
			uint32_t total = uint32_t(buffer[0]) + (uint32_t(buffer[1]) << 8) + (uint32_t(buffer[2]) << 16) +
			                 (uint32_t(buffer[3]) << 24);
			int_fast16_t result = (total / 273);
			// blur color
			if (i >= 2) {
				*(target_bitmap + bytes_per_pixel * (width * j + i - 2) + 2) = result;
				*(target_bitmap + bytes_per_pixel * (width * j + i - 2) + 1) = result;
				*(target_bitmap + bytes_per_pixel * (width * j + i - 2) + 0) = result;
			} else {
			}
		}
	}
